...
export default function TaskManager() {
    ...
    // Hydration fix: only access localStorage in useEffect
    useEffect(() => {
        const storedEmail = localStorage.getItem('userEmail');
        const storedToken = localStorage.getItem('accessToken');

        // Nếu chưa có token, redirect về login ngay
        if (!storedToken) {
            router.push('/login');
            return;
        }
    ...
    // Prevent hydration mismatch by returning null until client-side data is ready
