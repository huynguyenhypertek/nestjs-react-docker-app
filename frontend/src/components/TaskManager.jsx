...
export default function TaskManager() {
  ...
  const fetchTasks = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/tasks?page=${page}&limit=5&search=${search}`, {
                    ...
                    <div key={t.id} style={{...taskCard, flexDirection: 'column', alignItems: 'flex-start', gap: '10px'}}>
                        ...
                        {t.imageProof && (
                            <div style={proofContainer}>
                                <img 
                                  src={`http://localhost:3000/files/${t.imageProof}`} 
                                  alt="Minh chứng" 
                                  style={proofImage} 
                                />
...
const pageBtn = { padding: '8px 20px', backgroundColor: 'white', border: '1px solid #e2e8f0', borderRadius: '8px', cursor: 'pointer', fontWeight: '600', color: COLORS.textMain };
...
const inputStyle = { flex: 1, padding: '15px 25px', borderRadius: '12px', border: '1px solid #e2e8f0', outline: 'none', fontSize: '15px', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' };
const addBtn = { padding: '0 30px', backgroundColor: COLORS.primary, color: 'white', border: 'none', borderRadius: '12px', fontWeight: '600', cursor: 'pointer', transition: 'opacity 0.2s' };
const listContainer = { display: 'flex', flexDirection: 'column', gap: '12px', paddingBottom: '40px' };
const taskCard = { backgroundColor: 'white', padding: '18px 25px', borderRadius: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 2px 4px rgba(0,0,0,0.02)', border: '1px solid #f1f5f9', transition: 'transform 0.1s' };
const checkbox = { width: '22px', height: '22px', borderRadius: '6px', border: '2px solid #cbd5e1', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: '0.2s' };
...
const avatar = { width: '40px', height: '40px', borderRadius: '10px', backgroundColor: '#e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', color: COLORS.primary, flexShrink: 0 };
...
const logoutBtn = { width: '100%', padding: '10px', backgroundColor: '#fff', color: COLORS.danger, border: `1px solid ${COLORS.danger}`, borderRadius: '10px', fontWeight: '600', cursor: 'pointer', marginTop: '10px', fontSize: '13px' };
